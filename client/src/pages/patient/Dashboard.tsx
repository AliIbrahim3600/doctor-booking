...
const Dashboard = () => {
  ...
  const nextAppointment =
    confirmedAppointments.length > 0 ? confirmedAppointments[0] : appointments.length > 0 ? appointments[0] : null;

  // Generate dynamic days array for calendar display
  ...
  return (
    <div className="p-4 md:p-8 max-w-[1400px] w-full mx-auto pb-10 fade-in">
            ...
            {nextAppointment ? (
              <>
                <div>
                  ...
                  <h2 className="text-2xl md:text-3xl font-headline font-extrabold mb-2">
                    {nextAppointment.notes || "Medical Consultation"}
                  </h2>
                  <p className="text-white/80 font-medium">
                    {nextAppointment.doctorName} • {nextAppointment.speciality}
                  </p>
                </div>
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pt-6">
                  ...
                  <button onClick={() => navigate('/patient/records')} className="bg-white text-primary px-6 py-3 rounded-xl font-bold hover:bg-surface transition-colors cursor-pointer active:scale-95 shadow-md">
                    Prepare for Visit
                  </button>
                </div>
              </>
          ...
          <div className="space-y-4">
            {appointments.length > 0 ? (
              appointments.slice(0, 3).map((appt) => (
                <div
                  key={appt._id}
                  className={`bg-surface-container-lowest rounded-2xl p-5 md:p-6 border border-outline-variant/20 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 ${
                ...
                >
                    ...
                    <div>
                      ...
                      <p
                        className={`text-xs font-medium flex items-center gap-1.5 ${
                          appt.status === "cancelled" ? "text-error" : "text-outline-variant"
                        }`}
                      >
                        <span className="material-symbols-outlined text-[14px]">
                          {appt.status === "cancelled" ? "cancel" : "event"}
                        </span>
                  ...
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-2">
                    ...
                    {appt.status === "cancelled" && (
                      <button onClick={() => Swal.fire('Notice', 'To reschedule, please visit the Find Doctors page and book a new time.', 'info')} className="px-4 py-2 bg-primary-fixed/20 text-primary text-xs font-bold rounded-lg hover:bg-primary hover:text-white transition-colors cursor-pointer">
                        Reschedule
                      </button>
                    )}
                  </div>
          ...
          <div className="bg-surface-container-lowest rounded-3xl p-6 border border-outline-variant/20 shadow-[0_8px_30px_-4px_rgba(0,61,155,0.04)]">
            ...
            <div className="grid grid-cols-7 text-center mb-4">
              ...
              {calendarDays.map((day) => {
                const hasAppt = appointmentDates.includes(day);
                const isToday = day === todayDate;

                return (
                  <div key={day} className="p-1 relative flex items-center justify-center">
                    <span
                      className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition-colors cursor-default ${
                        isToday
            ...
            <div className="pt-6 border-t border-outline-variant/20 mt-4">
              ...
              {nextAppointment && nextAppointment.status !== "cancelled" ? (
                ...
                <p className="text-xs text-on-surface-variant italic">No agenda items lined up.</p>
              )}
            </div>
          </div>
        </div>
